...
vosk_interface = os.environ.get('VOSK_SERVER_INTERFACE', '192.168.10.72')
        ...
        while True:
            ...
            for fr in frames:
                for rfr in self.__resampler.resample(fr):
                    data = bytes(rfr.planes[0])[:rfr.samples * 2]

                    print(data)

                    response = requests.post('http://192.168.10.72:5000/convert_audio', data=data)
...
if __name__ == '__main__':
    ...
    web.run_app(app, host="192.168.10.72", port=vosk_port, ssl_context=ssl_context)
